...
export function Sidebar() {
  ...
  const [userRole, setUserRole] = useState("Anggota Tim");

  useEffect(() => {
    async function getUser() {
      try {
        const res = await fetchUserProfile();
        setUserName(res.user.username || "Pengguna");
        setUserRole(res.user.role || "Anggota Tim");
      } catch {
        ...
        if (typeof window !== "undefined") {
          ...
          if (profile) {
            try {
              ...
              setUserRole(user.role || "Anggota Tim");
            } catch {}
          }
        }
      }
        ...
        <div className="flex-shrink-0 flex border-t border-teal-700 p-4">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              ...
              <p className="text-xs font-medium text-teal-200">{userRole}</p>
            </div>
          </div>
        </div>
      </div>
