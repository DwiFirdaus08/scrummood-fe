...
export function PersonalSuggestions({
    ...
    async function fetchSuggestions() {
      ...
      try {
        const res = await fetchWithAuth(
          "https://scrummood-be-production.up.railway.app/api/suggestions/personal"
        );
        setSuggestions(res);
      } catch (e: any) {
  ...
  if (
    ...
    return <div>Tidak ada saran ditemukan.</div>;
      ...
      {suggestions.suggestions.map((item: any) => (
        <Card key={item.id} className="mb-4">
          <CardContent>
            ...
            {item.priority && (
              <Badge variant="outline">Prioritas: {item.priority}</Badge>
            )}
          </CardContent>
        </Card>
      ))}
