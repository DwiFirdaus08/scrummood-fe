...
export function MeetingChat({ messages, onSend, user }: MeetingChatProps) {
  ...
  const getEmotionIndicator = (emotion?: Message["emotion"]) => {
    ...
    return (
      <div
        className={`text-xs px-1.5 py-0.5 rounded-full flex items-center ${color}`}
      >
        ...
        <div className="space-y-4">
          {messages.map((message) => (
            <div
              key={message.id}
              className={`flex ${
                message.isSystem ? "justify-center" : "gap-3"
              }`}
            >
              {message.isSystem ? (
                ...
                <>
                  ...
                  <div className="flex flex-col">
                    ...
                    <div className="bg-gray-100 px-3 py-2 rounded-lg mt-1 text-sm">
                      {message.content}
                    </div>
                  </div>
                </>
