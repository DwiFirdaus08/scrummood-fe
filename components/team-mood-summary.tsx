...
export function TeamMoodSummary({ data, summary }: TeamMoodSummaryProps) {
  if (!data || data.length === 0) {
    return <div className="text-xs text-muted-foreground">Belum ada data mood tim.</div>
  }
  return (
      ...
      {data.map((member) => (
        <div key={member.id} className="flex items-center space-x-4">
          <Avatar>
            ...
            <AvatarFallback>{member.initials || (member.name ? member.name.split(" ").map((n) => n[0]).join("") : "U")}</AvatarFallback>
          </Avatar>
      ...
      {summary && (
        <div className="pt-4 border-t">
          ...
          <div className="grid grid-cols-5 gap-2 text-center">
            {Object.entries(summary).sort(([,a],[,b])=>(b as number)-(a as number)).map(([emo, val]) => (
              <div key={emo} className="flex flex-col items-center">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-lg mb-1 ${getMoodColor(emo as MoodData["mood"])}`}>{getMoodEmoji(emo as MoodData["mood"])}</div>
                <span className="text-xs">{Math.round((val as number)*100)}%</span>
              </div>
            ))}
          </div>
        </div>
