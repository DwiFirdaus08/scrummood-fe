...
export function VoiceEmotionDetector({
  ...
  const calculateSpeakingRate = (volume: number): number => {
    ...
    return Math.min(
      speechHistory.length > 0
        ? speechHistory.filter((v) => v > VOLUME_THRESHOLD).length /
  ...
  const analyzeEmotionFromFeatures = (
    features: typeof audioFeatures
  ): VoiceEmotionData => {
    const {
      pitch,
      energy,
    ...
    const emotionScores = {
      happy: calculateHappyScore(
        normalizedPitch,
        normalizedEnergy,
        normalizedVolume,
        speakingRate
      ),
      excited: calculateExcitedScore(
        normalizedPitch,
        normalizedEnergy,
        speakingRate,
        normalizedZCR
      ),
      calm: calculateCalmScore(
        normalizedPitch,
        normalizedEnergy,
        normalizedVolume,
        speakingRate
      ),
      neutral: calculateNeutralScore(
        normalizedPitch,
        normalizedEnergy,
        normalizedVolume
      ),
      stressed: calculateStressedScore(
        normalizedPitch,
        normalizedEnergy,
        speakingRate,
        normalizedZCR
      ),
      angry: calculateAngryScore(
        normalizedPitch,
        normalizedEnergy,
        normalizedSpectral,
        normalizedZCR
      ),
      sad: calculateSadScore(
        normalizedPitch,
        normalizedEnergy,
        normalizedVolume,
        speakingRate
      ),
      frustrated: calculateFrustratedScore(
        normalizedPitch,
        normalizedEnergy,
        speakingRate,
        normalizedZCR
  ...
  const calculateFrustratedScore = (
    pitch: number,
    energy: number,
    speakingRate: number,
    zcr: number
